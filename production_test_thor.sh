...
function test_menu {
	...
	while $continue_test; do
		...
		case $choice in
			...
			7 )
				echo "M.2 Key-E Test"
				gnome-terminal -- $SCRIPTS_FOLDER/test_key_e_thor.sh
				;;
			8 )
				echo "M.2 Key-B Test"
				gnome-terminal -- $SCRIPTS_FOLDER/test_key_b_thor.sh
				;;
			9 )
				echo "RS232 Test"
				check_nvgetty_service
				sudo gnome-terminal -- $SCRIPTS_FOLDER/test_rs232_thor.sh
				;;
			10 )
				echo "RS422 Test"
				check_nvgetty_service
				sudo gnome-terminal -- $SCRIPTS_FOLDER/test_rs422_thor.sh
				;;
			11 )
				echo "RS485 Write Test"
				check_nvgetty_service
				sudo gnome-terminal -- $SCRIPTS_FOLDER/test_rs485_write_thor.sh
				;;
			12 )
