...
sudo echo low > /sys/class/gpio/$HALF_FULL/direction
sudo echo low > /sys/class/gpio/$RS422_232/direction
sudo echo high > /sys/class/gpio/$TX_ENABLE/direction
sudo echo low > /sys/class/gpio/$RX_DISABLE/direction

sudo gtkterm -p /dev/ttyAMA10 -s 115200
